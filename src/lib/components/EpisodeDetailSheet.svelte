...
let sheetTop = $state(window.innerHeight * 0.3);
...
// When sheetTop is 0, content is scrollable
...
function handleTouchStart(e: TouchEvent) {
	...
	// Pull-down-to-collapse is detected in handleTouchMove.
	if (sheetTop === 0) {
		isDragging = false;
		return;
	}
...
function handleTouchMove(e: TouchEvent) {
	...
	// Not dragging sheet — check if we should start (pull down from scroll top)
	if (!isDragging) {
		if (sheetTop === 0 && contentEl && contentEl.scrollTop <= 0 && dy > 5) {
			// Start dragging the sheet down
			isDragging = true;
			dragStartY = e.touches[0].clientY;
			dragStartTop = 0;
		}
...
<div class="fixed inset-0 z-50">
	...
	<div
		...
		in:fly={{ y: window.innerHeight * 0.7, duration: 300, easing: cubicOut }}
		out:fly={{ y: window.innerHeight * 0.7, duration: 200, easing: cubicIn }}
	...
	>
		...
		<!-- Content -->
		<div
			class="flex-1 px-5 pb-4 {isFullyExpanded ? 'overflow-y-auto' : 'overflow-hidden'}"
			bind:this={contentEl}
		>
			<!-- Header: cover + info -->
			...
			{#if episode.description}
				<div class="text-sm text-text-secondary leading-relaxed episode-description">
